...
void expirySort(int catNo)
{
    ...
    if (catNo == 4)
    {
        *catalogueSingleCategory = catalogueSingleCategory->sortTo("expiry");
...
void welcome()
{
    ...
    LinkedList<PriceDataType> *catalogueSingleCategoryExpiry;
    ...
    if (x == 2)
    {
        ...
        displayAllProducts<PriceDataType>(*catalogueSingleCategoryPrice, "customer");
    }
    // admin
    // Features include: 1. Insert/Delete item 2. Sort by expiry/price
    else if (x == 1)
    {
        *catalogueSingleCategoryExpiry = catalogueSingleCategoryPrice->sortTo("expiry");
        //LinkedList<PriceDataType> productsExpiry = insertProduceProducts<PriceDataType>("expiry");
        displayAllCategories<PriceDataType>("expiry");
        displayAllProducts<PriceDataType>(*catalogueSingleCategoryExpiry, "admin");
    }
    return;
}
